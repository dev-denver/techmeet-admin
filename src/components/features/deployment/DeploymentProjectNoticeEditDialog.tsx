...
interface DeploymentProjectNoticeEditDialogProps {
  notice: DeploymentProjectNotice | null;
  onOpenChange: (open: boolean) => void;
}

export function DeploymentProjectNoticeEditDialog({ notice, onOpenChange }: DeploymentProjectNoticeEditDialogProps) {
  ...
  const form = useForm<DeploymentProjectNoticeCreateInput>({
    ...
    defaultValues: { project_id: "", transfer_notice: "", notice_date: "", main_manager: "" },
  });
  ...
  async function onSubmit(values: DeploymentProjectNoticeCreateInput) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/deployment/notices/${notice.id}`, {
        method: "PUT",
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/deployment/notices/${notice.id}`, { method: "DELETE" });
      const json = await res.json();
