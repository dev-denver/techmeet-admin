...
interface SmMemberEditDialogProps {
  member: SmMember | null;
  onOpenChange: (open: boolean) => void;
}

export function SmMemberEditDialog({ member, onOpenChange }: SmMemberEditDialogProps) {
  ...
  async function onSubmit(values: SmMemberCreateInput) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/deployment/sm-members/${member.id}`, {
        method: "PUT",
      ...
      toast.success("SM 인원 정보가 수정되었습니다.");
      setIsEditing(false);
      onOpenChange(false);
      router.refresh();
    } catch (e) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/deployment/sm-members/${member.id}`, { method: "DELETE" });
      const json = await res.json();
      if (!json.success) throw new Error(json.error?.message ?? "삭제 실패");
      toast.success("SM 인원이 삭제되었습니다.");
